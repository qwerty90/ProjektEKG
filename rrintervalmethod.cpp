...
vector<classification> RRIntervalMethod::classifyIntervals(vector<double> RRIntervals){
...
void RRIntervalMethod::countTransitions(vector<classification> classifiedIntervals){
    std::fill(&markovTable[0][0],&markovTable[3][0],0.0);
    for(auto it = classifiedIntervals.begin(); it != classifiedIntervals.end()-1; ++it){
