...
\begin{tikzpicture}
	\begin{class}[text width = 8cm]{EcgStAnalyzer}{0,0}
		...
		\operation{+ setLevelThreshold(value) : void}
		...
		\operation{+ analyze(ecgSamples : const QVector<double>\&, \\rVec : const QVector<EcgSampleIter>\&, \\jVec : const QVector<EcgSampleIter>\&, \\tEndVec : const QVector<EcgSampleIter>\&, \\sampleFreq : double) : bool}
		...
		\operation{- prepareData(ecgSamples : const QVector<double>\&, \\rVec : const QVector<EcgSampleIter>\&, \\jVec : const QVector<EcgSampleIter>\&, \\tEndVec : const QVector<EcgSampleIter>\&) : void}
	\end{class}
	
	\begin{class}[text width = 6.6cm]{EcgStDescriptor}{8,0}
	...
	\begin{class}[text width = 5cm]{<<enumeration>> AlgorithmType}{8,-15}
		\attribute{LINEAR}
		\attribute{QUADRATIC}
	\end{class}

	\begin{class}[text width = 5cm]{<<enumeration>> ErrorType}{8,-18}
		\attribute{NO\_ERROR}
		\attribute{NO\_SAMPLES\_PROVIDED}
		\attribute{INSUFFICIENT\_DATA}
	\end{class}

	\begin{class}[text width = 5cm]{<<enumeration>> EcgStPosition}{8,-7}
		\attribute{ST\_POS\_NORMAL}
		\attribute{ST\_POS\_ELEVATION}
		\attribute{ST\_POS\_DEPRESSION}
	\end{class}
	
	\begin{class}[text width = 5cm]{<<enumeration>> EcgStShape}{8,-10.5}
	...
	\aggregation{EcgStAnalyzer}{0..*}{output}{EcgStDescriptor}
\end{tikzpicture}